    ...
    bool color_octave)
{
  ...
  for(it = keypoints.begin(); it != keypoints.end(); ++it)
  {
    ...
    // angle is in [0..360]; assume it is in ^y x> axes
    // (while image is in vy x> axes)
    float o = (float)((360. - it->angle) * PI / 180.);
    ...
    int r2 = (int)(s * sin(o) + it->pt.y + 0.5);

    const CvScalar *color;
    if(!color_octave || it->octave > 3)
      color = &colors[3];
    else
