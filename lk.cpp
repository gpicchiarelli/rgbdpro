...
{    
    ...
    bool flag_l3d=false,flag_lrgb=false,flag_stats=false;
    if(nNumberofArgs > 0){
        ...
        if ( find(parametri.begin(), parametri.end(), "-h") != parametri.end()){
            ...
            if(flag_stats){
                ...
                stats* owl = new stats("report_3d.rep",registro_interno,80,"3D");
                cout << owl->toString();
                stats* owl2 = new stats("report_rgb.rep",registro_interno,80,"RGB");
                cout << owl2->toString();
            }
...
void loopClosingRGB(BoWFeatures &features){
    ...
    int const OFFSET = 80;
    double const MATCH_THRESHOLD = 0.49;
