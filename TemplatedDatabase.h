...
template<>
struct convert< vector<int> > {
    static Node encode(const vector<int>& rhs) {
        Node node;
    ...
    static bool decode(const Node& node, vector<int>& rhs) {
        if(!node.IsSequence()){
            return false;
        }
        for(int y=0;y<rhs.size();y++){
            cout << node[y].as<int>() << endl;
            rhs.push_back(node[y].as<int>());
        }
        return true;
    }
};
                                             ...
                                             const std::string &name)
{
    ...
    if(m_use_di)
    {
        ...
        string wrk = decompressed.str();
        
        YAML::Node ffl = YAML::Load(wrk);
        YAML::Node di_t = ffl[name];
        YAML::Node fi_t = di_t["directIndex"];
        
        FeatureVector::iterator dit;
        for(EntryId eid = 0; eid < fn.size(); ++eid)
        {
            cv::FileNode fe = fn[eid];
            YAML::Node ei_t = fi_t[eid];
            m_dfile[eid].clear();
            for(unsigned int i = 0; i < fe.size(); ++i)
            {
                ...
                vec_int aux;                
                YAML::Node buff = ei_t[i];
                YAML::Node buff_t = buff["features"];
                aux =  buff_t[0].as<vec_int>();
