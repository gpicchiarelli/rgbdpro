...
public:
   ...
   * @return word id
   */
  WordId transform(const TDescriptor& feature) const;
  ...
  /**
   * Loads the vocabulary from a file storage structure
   * @param f
   * @param name node name
   */
  void load(cv::FileStorage &f, const std::string &name = "vocabulary");
...
protected:
   ...
   */
  void transform(const TDescriptor &feature,
   ...
   * @param id (out) word id
   */
  void transform(const TDescriptor &feature, WordId &id) const;
    ...
    while(goon)
    {
      ...
      else
      {
        ...
        for(unsigned int i = 0; i < clusters.size(); ++i)
        {
          ...
          cluster_descriptors.reserve(groups[i].size());
          ...
          F::meanValue(cluster_descriptors, clusters[i]);
        }
      ...
      for(fit = descriptors.begin(); fit != descriptors.end(); ++fit)
      {
        ...
        {
          double dist = F::distance(*(*fit), clusters[i]);
          if(dist < best_dist)
          {
            best_dist = dist;
            icluster = i;
  ...
  const vector<pDescriptor> &pfeatures, vector<TDescriptor> &clusters) const
{
  ...
  clusters[0] = *pfeatures[ifeature];
  int used_clusters = 1;

  vector<double> distances;
  vector<int> ifeatures;

  distances.reserve(pfeatures.size());
  ...
  while(used_clusters < m_k)
  {
    ...
    for(fit = pfeatures.begin(); fit != pfeatures.end(); fit++)
    {
      ...
      {
        double min_dist = F::distance(*(*fit), clusters[0]);
    ...
    // 3.
    double dist_sum = std::accumulate(distances.begin(), distances.end(), 0.0);
    ...
    if(dist_sum > 0)
    {
      ...
      if(dit == distances.end())
        dit = distances.begin() + distances.size()-1;

      ifeature = ifeatures[dit - distances.begin()];

      assert(!feature_used[ifeature]);

      clusters[used_clusters] = *pfeatures[ifeature];
      feature_used[ifeature] = true;
      used_clusters++;
  ...
  if(used_clusters < m_k)
    clusters.resize(used_clusters);
}
  ...
  (const vector<vector<TDescriptor> > &training_features)
{
  ...
  else if(m_weighting == IDF || m_weighting == TF_IDF)
  {
    ...
    for(mit = training_features.begin(); mit != training_features.end(); ++mit)
    {
      ...
      // #### verificar que Ni solo se tiene que contar una vez por palabra
      // e imagen
...
{
  if(m_words.empty() || m_nodes.empty() || m_nodes[0].isLeaf())
  {
    return 0;
  }
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::load(cv::FileStorage &f,
  const std::string &name)
{
  m_words.clear();
  m_nodes.clear();

  cv::FileNode fvoc = f[name];
